...
void GuiWindowAbout(GuiWindowAboutState *state)
{
    const char *windowAboutText = "#191#About rIconPacker";
    const char *lblDescriptionText = "A simple and easy-to-use icons packer";
    const char *lblNameVersionText = "rIconPacker v1.1";
    ...
    if (state->windowAboutActive)
    {
        ...
        state->windowAboutActive = !GuiWindowBox((Rectangle){ state->position.x + 0, state->position.y + 0, state->windowWidth, state->windowHeight }, TextFormat("%s %s", windowAboutText, mode));
